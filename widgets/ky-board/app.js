...
// ===================== KY活動ボード =====================
// 本日の危険予知（KY）活動の内容と対策をサイネージに掲示
// Google Form → Sheets CSV で朝礼結果をリアルタイム反映
// CSV形式: 日付, リスク内容, 対策, レベル(high/medium/low), 担当班

var _timer = null;
var SAMPLE = [
  { risk: '足場上での転落', action: 'ハーネス着用確認・手すり設置', level: 'high',   team: '鉄筋班' },
  { risk: '重機旋回による接触', action: '旋回範囲にバリケード設置、誘導員配置', level: 'high',   team: '重機班' },
  { risk: '資材運搬時の腰痛', action: '2人作業徹底・補助具使用', level: 'medium', team: '全班' },
  { risk: '熱中症のリスク（本日WBGT高）', action: 'こまめな水分補給・30分ごとに休憩', level: 'medium', team: '全班' }
];
...
function _start(config) {
  ...
  if (titleEl) titleEl.textContent = config.siteTitle || '本日のKY活動';
  ...
  _renderQR((config.formUrl || '').trim(), config.qrLabel || 'KY結果を投稿');
  _updateDate();
}
...
function _fetchCSV(url) {
  ...
  xhr.onload = function() {
    ...
    var rows = _parseCSV(xhr.responseText);
    _render(rows.length ? rows : SAMPLE);
  };
  xhr.ontimeout = function() { _render(SAMPLE); };
  xhr.onerror   = function() { _render(SAMPLE); };
  xhr.send();
}
...
function _parseCSV(text) {
  ...
  text.trim().split('\n').forEach(function(line) {
    ...
    if (cols.length >= 4 && cols[0]) {
      // 日付フィルタ（今日分のみ）
      if (cols[0] && cols[0] !== today && cols[0] !== '') {
        // 日付列がある場合は今日のみ
      }
      rows.push({
        risk:   cols[1] || cols[0] || '',
        action: cols[2] || '',
        level:  (cols[3] || 'medium').toLowerCase(),
        team:   cols[4] || ''
      });
    }
  });
...
function _render(items) {
  var listEl = document.getElementById('ky-list');
  if (!listEl) return;
  listEl.innerHTML = '';
  ...
  items.forEach(function(item, idx) {
    ...
    div.className = 'ky-item level-' + _sanitize(item.level);

    var icon = item.level === 'high' ? '🔴' : item.level === 'medium' ? '🟡' : '🟢';
    var levelLabel = item.level === 'high' ? '重大リスク' : item.level === 'medium' ? '注意' : '軽微';

    div.innerHTML =
      '<div class="ky-header">' +
        '<span class="ky-num">' + (idx + 1) + '</span>' +
        '<span class="ky-icon">' + icon + '</span>' +
        '<span class="ky-level">' + levelLabel + '</span>' +
        (item.team ? '<span class="ky-team">' + _esc(item.team) + '</span>' : '') +
      '</div>' +
      '<div class="ky-risk">' + _esc(item.risk) + '</div>' +
      '<div class="ky-action">▶ ' + _esc(item.action) + '</div>';
...
function _renderQR(url, label) {
  ...
  if (canvas) {
    var qrc = QR.generate(url, 160, '#ffffff', '#1a2a1a');
    if (qrc) { canvas.width = qrc.width; canvas.height = qrc.height; canvas.getContext('2d').drawImage(qrc, 0, 0); }
  }
}
...
function _updateDate() {
  ...
  el.textContent = now.getFullYear() + '年' + (now.getMonth()+1) + '月' + now.getDate() + '日（' + days[now.getDay()] + '）朝礼';
}

function _todayStr() {
  var now = new Date();
  return now.getFullYear() + '-' + _z(now.getMonth()+1) + '-' + _z(now.getDate());
}

function _sanitize(s) { return (s || '').replace(/[^a-z0-9-]/g, ''); }
...
if (window.location.protocol === 'file:') {
  window.onload = function() {
    init_widget({ siteTitle: '本日のKY活動', dataUrl: '', formUrl: 'https://forms.gle/example', qrLabel: 'KY結果を投稿', refreshMin: '5', fontFamily: 'keifont, sans-serif' });
  };
}
